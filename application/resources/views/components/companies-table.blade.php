            ...
            <tr tabindex="0"
                ...
                <td class="pl-24"
                    ...
                    <div class="flex items-center">
                        <img src="{{ $entry->logo ? asset('storage/' . $entry->logo) : '/images/default.png' }}"
                             alt="{{ $entry->name }}"
                ...
                <td class=""
                    ...
                    <div class="flex items-center pl-5">
                        ...
                        <a href="#"
                           target="_blank">
                            <!-- Inserir a URL para visualização da empresa -->
                ...
                <td class="pl-24"
                    ...
                    <div class="flex items-center">
                        ...
                        <p class="text-sm leading-none text-gray-600 ml-2">{{ $entry->category->name }}
                        </p>
                    </div>
                </td>

                <td class="pl-5"
                    title="Usuário que cadastrou a Empresa">
                    <div class="flex items-center">
                        ...
                        <p class="text-sm leading-none text-gray-600 ml-2">{{ $entry->user->name }}</p>
                    </div>
                </td>


                <td class="pl-5"
                    title="Imagens atribuidas à Empresa">
                    <div class="flex items-center">
                        <svg fill="#000000"
                             viewBox="0 0 24 24"
                             xmlns="http://www.w3.org/2000/svg"
                        ...
                        <p class="text-sm leading-none text-gray-600 ml-2">{{ count($entry->images) }}
                        </p>
                    </div>
                </td>
                ...
                <form data-id="{{ $entry->id }}"
                      ...
                      @can('manage_companies')
                        action=" {{ route('admin.company.delete', ['id' => $entry->id]) }}"
                        @elsecan('manage_business')
                        action=" {{ route('partner.business.delete', ['id' => $entry->id]) }}"
                        @else
                        action=" {{ route('partner.business.delete', ['id' => $entry->id]) }}"
                        @endcan
                      method="POST">
                    ...
                    <td class="pl-4 text-right">
                        @can('manage_companies')
                            <a href="{{ route('admin.company.show', ['id' => $entry->id]) }}"
                               class="focus:ring-2 focus:ring-offset-2 focus:ring-blue-950 text-sm leading-none text-gray-600 py-3 px-5 mx-0.5 bg-gray-100 rounded hover:bg-gray-200 focus:outline-none">
                                Visualizar
                            </a>
                            @if ($entry->status === 'pending_approval')
                                <a href="#0"
                                   onclick="event.preventDefault()"
                                   data-url="{{ route('admin.company.manage-ban', ['id' => $entry->id]) }}"
                                   class="focus:ring-2 focus:ring-offset-2 focus:ring-blue-950 text-sm leading-none text-gray-600 py-3 px-5 mx-0.5 bg-gray-100 rounded hover:bg-gray-200 focus:outline-none ban-button">
                                    Aprovar
                                </a>
                            @endif
                        @elsecan('manage_business')
                            <a href="{{ route('partner.business.gallery', ['id' => $entry->id]) }}"
                               class="focus:ring-2 focus:ring-offset-2 focus:ring-blue-950 text-sm leading-none text-gray-600 py-3 px-5 bg-gray-100 rounded hover:bg-gray-200 focus:outline-none">Gerir
                                Imagens</a>
                            <a href="{{ route('partner.business.show', ['id' => $entry->id]) }}"
                               class="focus:ring-2 focus:ring-offset-2 focus:ring-blue-950 text-sm leading-none text-gray-600 py-3 px-5 mx-0.5 bg-gray-100 rounded hover:bg-gray-200 focus:outline-none">
                                Visualizar
                            </a>
                        @endcan
                        @can('manage_companies')
                            @if (in_array($entry->status, ['active', 'banned']))
                                <a href="#"
                                   onclick="event.preventDefault()"
                                   data-url="{{ route('admin.company.manage-ban', ['id' => $entry->id]) }}"
                                   class="focus:ring-2 focus:ring-offset-2 focus:ring-blue-950 text-sm leading-none text-gray-600 py-3 px-5 mx-0.5 bg-gray-100 rounded hover:bg-gray-200 focus:outline-none
                                                        ban-button">
                                    {{ $entry->status == 'banned' ? 'Liberar' : 'Banir' }}
                                </a>
                            @endif
                        @endcan
                        @can('manage_business')
                            @can('manage_products')
                                <a href="{{ route('partner.products.index', ['company_id' => $entry->id]) }}"
                                   class="focus:ring-2 focus:ring-offset-2 focus:ring-blue-950 text-sm leading-none text-gray-600 py-3 px-5 mx-0.5 bg-gray-100 rounded hover:bg-gray-200 focus:outline-none">
                                    Produtos
                                </a>
                            @endcan
                            @can('manage_services')
                                <a href="{{ route('partner.services.index', ['company_id' => $entry->id]) }}"
                                   class="focus:ring-2 focus:ring-offset-2 focus:ring-blue-950 text-sm leading-none text-gray-600 py-3 px-5 mx-0.5 bg-gray-100 rounded hover:bg-gray-200 focus:outline-none">
                                    Serviços
                                </a>
                            @endcan
                        ...
                        <a href="#"
                           onclick="event.preventDefault()"
                           data-method="DELETE"
                           data-url="{{ route('partner.business.delete', ['id' => $entry->id]) }}"
                           data-id="{{ $entry->id }}"
                           class="focus:ring-2 focus:ring-offset-2 focus:ring-red-300 text-sm leading-none text-gray-50 py-3 px-5 mx-0.5 bg-red-400 rounded hover:bg-red-500 focus:outline-none
                                                    company-confirm-delete">
                            Apagar
                        </a>
