    ...
    <div class="sm:px-6 w-full">
        ...
        <div class="bg-white py-4 md:py-7 px-4 md:px-8 xl:px-10">
            <div class="sm:flex items-center justify-between">
                ...
                <a href="{{ route('admin.plans.show', ['id' => 'new']) }}"
                                ...
                                <td class="pl-5"
                                    ...
                                    <div class="flex items-center">
                                        ...
                                        <p class="text-sm leading-none text-gray-600 ml-2">
                                            {{-- {{ \App\Models\User::plans($entry->name)->count() }} --}}
                                        </p>
                                    </div>
                                </td>
                                <form data-id="{{ $entry->id }}"
                                      class="confirm-delete-form"
                                      action="{{ route('admin.plans.delete', ['id' => $entry->id]) }}"
                                      method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <td class="pl-4 text-right gap-4">
                                        <a href="{{ route('admin.plans.show', ['id' => $entry->id]) }}"
                                        ...
                                        <button href="#"
                                                ...
                                                data-method="DELETE"
                                                data-url="{{ route('admin.plans.delete', ['id' => $entry->id]) }}"
                                                data-id="{{ $entry->id }}"
...
<script>
    $("#change-order").on("change", function() {
        ...
        window.location.href = "{{ route('admin.plans.index') }}?order=" + order;
    });
