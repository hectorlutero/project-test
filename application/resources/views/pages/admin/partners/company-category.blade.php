            ...
            <div class="flex items-center justify-between">
                <p tabindex="0"
                   class="focus:outline-none text-base sm:text-lg md:text-xl lg:text-2xl font-bold leading-normal text-gray-800">
                    Segmentações/Categorias de Parceiros</p>
                <div
            ...
            <div class="sm:flex items-center justify-between">
                <div class="flex items-center">
                    <x-dropdown-filter align="left"
                                       action="{{ route('admin.company-categories.index') }}" />
                </div>

                <a href="{{ route('admin.company-categories.show', ['id' => 'new']) }}"
                   class="focus:ring-2 focus:ring-offset-2 focus:ring-indigo-600 mt-4 sm:mt-0 inline-flex items-start justify-start px-6 py-3 bg-indigo-700 hover:bg-indigo-600 focus:outline-none rounded">
                    <p class="text-sm font-medium leading-none text-white">Adicionar Segmentação</p>
                </a>
            </div>
                    ...
                    <tbody>
                        @forelse($entries['list'] as $entry)
                            <tr tabindex="0"
                                ...
                                <td class=""
                                    title="O que esta segmentação pode acessar">
                                    <div class="flex items-center pl-5">
                                        <p class="text-base text-right font-medium leading-none text-gray-700 mr-2">
                                            {!! $entry->capabilities !!}
                                        </p>
                                    </div>
                                </td>
                                ...
                                <td class="pl-5"
                                    title="Icone atribuido à Segmentação">
                                    <div class="flex items-center">
                                        <img src="{{asset("images/".$entry->icon)}}" height="20" width="20">
                                    </div>
                                </td>

                                <form data-id="{{ $entry->id }}"
                                      class="confirm-delete-form"
                                      action="{{ route('admin.company-categories.delete', ['id' => $entry->id]) }}"
                                      method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <td class="pl-4 text-right">
                                        <a href="{{ route('admin.company-categories.show', ['id' => $entry->id]) }}"
                                        ...
                                        <button href="#"
                                                ...
                                                data-method="DELETE"
                                                data-url="{{ route('admin.company-categories.delete', ['id' => $entry->id]) }}"
                                                data-id="{{ $entry->id }}"
...
<script>
    $("#change-order").on("change", function() {
        ...
        window.location.href = "{{ route('admin.company-categories.index') }}?order=" + order;
    });
    $(document).on("click", ".confirm-delete", function() {
        $("#confirmation-modal-title").text("Apagar segmentação");
        $("#confirmation-modal-content").text("Tem certeza que deseja apagar esta segmentação?");
