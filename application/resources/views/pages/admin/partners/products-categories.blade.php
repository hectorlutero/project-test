            ...
            <div class="sm:flex items-center justify-between">
                <div class="flex items-center">
                    <x-dropdown-filter align="left"
                                       action="{{ route('admin.products-categories.index') }}" />
                </div>

                <a href="{{ route('admin.products-categories.show', ['id' => 'new']) }}"
                   class="focus:ring-2 focus:ring-offset-2 focus:ring-indigo-600 mt-4 sm:mt-0 inline-flex items-start justify-start px-6 py-3 bg-indigo-700 hover:bg-indigo-600 focus:outline-none rounded">
                    <p class="text-sm font-medium leading-none text-white">Adicionar Categoria de Produto</p>
                </a>
            </div>
                    ...
                    <tbody>
                        @forelse($entries['list'] as $entry)
                            <tr tabindex="0"
                                ...
                                <td class=""
                                    title="Nome da Categoria de Produto">
                                ...
                                <form data-id="{{ $entry->id }}"
                                      ...
                                      action="{{ route('admin.products-categories.delete', ['id' => $entry->id]) }}"
                                      method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <td class="pl-4 text-right">
                                        <a href="#0"
                                           class="focus:ring-2 focus:ring-offset-2 focus:ring-blue-950 text-sm leading-none text-gray-600 py-3 px-5 bg-gray-100 rounded hover:bg-gray-200 focus:outline-none">Gerir
                                            Imagens</a>
                                        <a href="{{ route('admin.products-categories.show', ['id' => $entry->id]) }}"
                                        ...
                                        <button href="#"
                                                ...
                                                data-method="DELETE"
                                                data-url="{{ route('admin.products-categories.delete', ['id' => $entry->id]) }}"
                                                data-id="{{ $entry->id }}"
...
<script>
    $("#change-order").on("change", function() {
        ...
        window.location.href = "{{ route('admin.products-categories.index') }}?order=" + order;
    });
    $(document).on("click", ".confirm-delete", function() {
        $("#confirmation-modal-title").text("Apagar categoria de produto");
        $("#confirmation-modal-content").text("Tem certeza que deseja apagar esta categoria de produto?");
