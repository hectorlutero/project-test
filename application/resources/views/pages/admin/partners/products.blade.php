<x-app-layout>
            ...
            <div class="sm:flex items-center justify-between">
                ...
                @can('manage_business')
                    <div class="table-actions">
                        <a href="{{ route('admin.product.show', ['id' => 'new']) }}"
                        ...
                        <button href=""
                                ...
                                data-method="POST"
                                data-url="{{ route('admin.product.import') }}"
                    ...
                    <tbody>
                        ...
                        @forelse($entries['list'] as $entry)
                            <tr tabindex="0"
                                ...
                                <td class="pl-5"
                                    title="Produto cadastrado pela empresa">
                                    <div class="flex items-center">
                                        ...
                                        <p class="text-sm leading-none text-gray-600 ml-2">
                                            {{ $entry->company_name ? $entry->company_name : '' }}</p>
                                    </div>
                                </td>
                                <td class="pl-5"
                                ...
                                <form data-id="{{ $entry->id }}"
                                      ...
                                      action="{{ route('admin.product.delete', ['id' => $entry->id]) }}"
                                      method="POST">
                                    @csrf
                                    <td class="pl-4 text-right gap-4">
                                        @if ($entry->status === 'DRAFT')
                                            <a class="focus:ring-2 focus:ring-offset-2 focus:ring-emerald-300 text-sm leading-none text-gray-50 py-3 px-5 mx-0.5 bg-emerald-500 rounded hover:bg-emerald-600 focus:outline-none cursor-pointer publish-item"
                                               data-url="{{ route('admin.product.save', ['id' => $entry->id]) }}"
                                               data-id="{{ $entry->id }}"
                                               data-method="POST"
                                               data-status="PUBLISHED"
                                               title="Publicar"><i class="fa fa-file-lines"></i></a>
                                        @else
                                            <a class="focus:ring-2 focus:ring-offset-2 focus:ring-amber-300 text-sm leading-none text-gray-50 py-3 px-5 mx-0.5 bg-amber-500 rounded hover:bg-amber-600 focus:outline-none cursor-pointer publish-item"
                                               data-url="{{ route('admin.product.save', ['id' => $entry->id]) }}"
                                               data-id="{{ $entry->id }}"
                                               data-method="POST"
                                               data-status="DRAFT"
                                               title="Rascunho"><i class="fa-brands fa-firstdraft"></i></a>
                                        @endif
                                        <a href="{{ route('admin.product.gallery', ['id' => $entry->id]) }}"
                                           class="focus:ring-2 focus:ring-offset-2 focus:ring-blue-950 text-sm leading-none text-gray-600 py-3 px-5 bg-gray-100 rounded hover:bg-gray-200 focus:outline-none">Gerir
                                            Imagens</a>
                                        <a href="{{ route('admin.product.show', ['id' => $entry->id]) }}"
                                        ...
                                        <button href="#"
                                                onclick="event.preventDefault()"
                                                data-method="DELETE"
                                                data-url="{{ route('admin.product.delete', ['id' => $entry->id]) }}"
                                                data-id="{{ $entry->id }}"
                                                class="focus:ring-2 focus:ring-offset-2 focus:ring-red-300 text-sm leading-none text-gray-50 py-3 px-5 mx-0.5 bg-red-400 rounded hover:bg-red-500 focus:outline-none confirm-delete">
                                            <i class="fa fa-trash mr-2"></i> Apagar</button>
                                    </td>
                                </form>
    ...
    @php
        $type = [
            ...
            'plural' => 'products',
            'route' => [
                'save' => 'admin.product.save.many',
                'delete' => 'admin.product.delete.many',
            ],
            'method' => [
                'save' => 'POST',
                'delete' => 'DELETE',
    ...
    {{-- <x-import-spreadsheet-modal :companies="$entries['companies']"
                                :type="$entries['type']" /> --}}
...
<script>
    $("#change-order").on("change", function() {
        ...
        window.location.href = "{{ route('admin.products.index') }}?order=" + order;
    });

    $(document).on("click", ".confirm-delete", function() {
        $("#confirmation-modal-title").text("Apagar produto");
        $("#confirmation-modal-content").text("Tem certeza que deseja apagar este produto?");
    ...
    $(document).on("click", ".publish-item", function() {
        ...
        let url = $("#confirmation-modal-destination-url").val($(this).data('url'));
