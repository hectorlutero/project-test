            ...
            <div class="sm:flex items-center justify-between">
                <div class="flex items-center">
                    <x-dropdown-filter align="left"
                                       action="{{ route('admin.services-tags.index') }}" />
                </div>

                <a href="{{ route('admin.services-tags.show', ['id' => 'new']) }}"
                   class="focus:ring-2 focus:ring-offset-2 focus:ring-indigo-600 mt-4 sm:mt-0 inline-flex items-start justify-start px-6 py-3 bg-indigo-700 hover:bg-indigo-600 focus:outline-none rounded">
                    <p class="text-sm font-medium leading-none text-white">Adicionar Tag de Serviço</p>
                </a>
            </div>
                        ...
                        @forelse($entries['list'] as $entry)
                            <tr tabindex="0"
                                ...
                                <form data-id="{{ $entry->id }}"
                                      ...
                                      action="{{ route('admin.services-tags.delete', ['id' => $entry->id]) }}"
                                      method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <td class="pl-4 text-right">
                                        <a href="{{ route('admin.services-tags.show', ['id' => $entry->id]) }}"
                                        ...
                                        <button href="#"
                                                ...
                                                data-method="DELETE"
                                                data-url="{{ route('admin.services-tags.delete', ['id' => $entry->id]) }}"
                                                data-id="{{ $entry->id }}"
...
<script>
    $("#change-order").on("change", function() {
        ...
        window.location.href = "{{ route('admin.services-tags.index') }}?order=" + order;
    });
    $(document).on("click", ".confirm-delete", function() {
        $("#confirmation-modal-title").text("Apagar tag de serviço");
        $("#confirmation-modal-content").text("Tem certeza que deseja apagar esta tag de serviço?");
