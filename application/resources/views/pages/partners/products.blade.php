<x-app-layout>
        ...
        <div class="bg-white py-4 md:py-7 px-4 md:px-8 xl:px-10">
            <div class="sm:flex items-center justify-between">
                <div class="flex items-ceter">
                    <x-dropdown-filter align="left"
                                       action="{{ route('partner.products.index') }}"
                                       :elements="$entries['filtersMenu']"
                                       :checkedOptions="$entries['checkedOptions']" />
                    <form action="{{ route('partner.products.search') }}"
                          method="get"
                ...
                <div class="table-actions">
                    @if ($entries['company_id'])
                        <a href="{{ route('partner.product.show', ['id' => 'new', 'company_id' => $entries['company_id']]) }}"
                        ...
                        <button href=""
                                ...
                                data-method="POST"
                                data-url="{{ route('partner.product.import', ['company_id' => $entries['company_id']]) }}"
            ...
            <div class="mt-7 overflow-x-auto">
                        ...
                        @forelse($entries['list'] as $entry)
                            <tr tabindex="0"
                                ...
                                <form data-id="{{ $entry->id }}"
                                      ...
                                      action="{{ route('partner.product.delete', ['id' => $entry->id]) }}"
                                      method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <td class="pl-4 text-right gap-4">
                                        <a href="{{ route('partner.product.gallery', ['id' => $entry->id]) }}"
                                           class="focus:ring-2 focus:ring-offset-2 focus:ring-blue-950 text-sm leading-none text-gray-600 py-3 px-5 bg-gray-100 rounded hover:bg-gray-200 focus:outline-none">Gerir
                                            Imagens</a>
                                        <a href="{{ route('partner.product.show', ['id' => $entry->id, 'company_id' => $entry->company_id]) }}"
                                        ...
                                        <button href="#"
                                                onclick="event.preventDefault()"
                                                data-method="DELETE"
                                                data-url="{{ route('partner.product.delete', ['id' => $entry->id]) }}"
                                                data-id="{{ $entry->id }}"
                ...
                <div class="w-full text-end">
                    <a href="{{ route('partner.product.export') }}"
    ...
    @php
        $type = [
            ...
            'plural' => 'products',
            'route' => [
                'save' => 'partner.product.save.many',
                'delete' => 'partner.product.delete.many',
            ],
            'method' => [
                'save' => 'POST',
                'delete' => 'DELETE',
...
<script>
    $("#change-order").on("change", function() {
        ...
        window.location.href = "{{ route('partner.products.index') }}?order=" + order;
    });
