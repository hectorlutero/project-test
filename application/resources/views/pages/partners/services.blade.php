<x-app-layout>
        ...
        <div class="bg-white py-4 md:py-7 px-4 md:px-8 xl:px-10">
            <div class="sm:flex items-center justify-between">
                <div class="flex items-ceter">
                    <x-dropdown-filter align="left"
                                       action="{{ route('partner.services.index') }}"
                                       :elements="$entries['filtersMenu']"
                                       :checkedOptions="$entries['checkedOptions']" />
                    <form action="{{ route('partner.services.search') }}"
                          method="get"
                ...
                <div class="table-actions">
                    @if ($entries['company_id'])
                        <a href="{{ route('partner.service.show', ['id' => 'new', 'company_id' => $entries['company_id']]) }}"
                           class="focus:ring-2 focus:ring-offset-2 focus:ring-indigo-600 mt-4 sm:mt-0 inline-flex items-start justify-start px-6 py-3 bg-indigo-700 hover:bg-indigo-600 focus:outline-none rounded justify-self-end">
                            <p class="text-sm font-medium leading-none text-white"><i class="fa fa-plus mr-2"></i>
                                Adicionar
                                Serviço</p>
                        </a>
                        <button href=""
                                ...
                                data-method="POST"
                                data-url="{{ route('partner.service.import') }}"
            ...
            <div class="mt-7 overflow-x-auto">
                            ...
                            <tr tabindex="0"
                                ...
                                <td class=""
                                    ...
                                    <div class="flex items-center pl-5">
                                        ...
                                        <a href="#"
                                           target="_blank">
                                            <!-- Inserir a URL para visualização da serviço -->
                                ...
                                <td class="pl-5"
                                    title="Serviço cadastrado pela empresa">
                                    <div class="flex items-center">
                                ...
                                <td class="pl-5"
                                    title="Imagens atribuidas à Serviço">
                                    <div class="flex items-center">
                                        <svg fill="#000000"
                                             viewBox="0 0 24 24"
                                             xmlns="http://www.w3.org/2000/svg"
                                ...
                                <form data-id="{{ $entry->id }}"
                                      ...
                                      action="{{ route('partner.service.delete', ['id' => $entry->id]) }}"
                                      method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <td class="pl-4 text-right gap-4">

                                        <a href="{{ route('partner.service.gallery', ['id' => $entry->id]) }}"
                                           class="focus:ring-2 focus:ring-offset-2 focus:ring-blue-950 text-sm leading-none text-gray-600 py-3 px-5 bg-gray-100 rounded hover:bg-gray-200 focus:outline-none">Gerir
                                            Imagens</a>
                                        <a href="{{ route('partner.service.show', ['id' => $entry->id, 'company_id' => $entry->company_id]) }}"
                                        ...
                                        <button href="#"
                                                onclick="event.preventDefault()"
                                                data-method="DELETE"
                                                data-url="{{ route('partner.service.delete', ['id' => $entry->id]) }}"
                                                data-id="{{ $entry->id }}"
                                            ...
                                            <i class="fa fa-trash mr-2"></i> Apagar</a>
                                    </td>
                                </form>
                ...
                <div class="w-full text-end">
                    <a href="{{ route('partner.service.export') }}"
    ...
    <x-import-spreadsheet-modal :company="$entries['company_id']"
                                :type="$entries['type']" />
    @php
        $type = [
            'singular' => 'service',
            'plural' => 'services',
            'route' => [
                'save' => 'partner.service.save.many',
                'delete' => 'partner.service.delete.many',
            ],
            'method' => [
                'save' => 'POST',
                'delete' => 'DELETE',
...
<script>
    $("#change-order").on("change", function() {
        ...
        window.location.href = "{{ route('partner.services.index') }}?order=" + order;
    });
    ...
    $(document).on("click", ".import-spreadsheet", function() {
        ...
        $("#import-spreadsheet-content").text("Tem certeza que deseja apagar este serviço?");
